...
Telemetry is opt-in, local, off by default, and never leaves the machine. When enabled, each debrief appends one record (capped at 60) covering outcome, duration, first contact, objectives, weapon shots and damage, damage in and out, civilian hits by source, item and ability use, KIA, payout, and deployed roles. At 61 the oldest record leaves (FIFO). Abort writes a thin record: `aborted`, duration, mission id, seed, deployed roles. A Balance dashboard aggregates those records. Win rate is won / (won + lost). Abort count and abort rate sit beside it. Export is a local JSON download. Clear is a two-step confirm. The log survives New Operation; Clear is how it dies. The log is not a campaign transaction: a reload on Debrief may keep a row for an outcome the campaign rolled back. [ADR-0015](architecture/adr-0015-telemetry-never-leaves-the-machine.md).
